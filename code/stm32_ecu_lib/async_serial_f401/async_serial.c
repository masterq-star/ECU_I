 ...
 *
 * Once the cubeMX & HAL initialisation is complete, the user calls asseInitialise() to initialise the serial channel.
 *
...
int asseTimeout = 500;		// the maximum wait time in mS
...
void asseISR(asseControlData *c){
	...
	if ((c->usart->SR & 0x40) != 0){
		...
		if (*c->txBufferPtr == 0){
			// end of transmission, clear the TC flag
			c->usart->SR &= ~0x40;
			c->txInProgress = 0;
		}
		else {
 ...
 *
 */
void asseInitialise(asseControlData *c, USART_TypeDef *usart, char terminator, char *rxBuffer, int rxBuffSize){
...
5) 12 May 2021 Modified for F401CC MCU.
